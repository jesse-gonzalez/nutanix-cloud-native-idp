...
resource "null_resource" "copy_os_image" {
  ...
  provisioner "local-exec" {
    command = "scp -i ${var.private_key_file_path} -r ${var.vm_user}@${data.nutanix_virtual_machine.build_vm_datasource.nic_list.0.ip_endpoint_list[0].ip}:~/image-builder/images/capi/output ."
  }
