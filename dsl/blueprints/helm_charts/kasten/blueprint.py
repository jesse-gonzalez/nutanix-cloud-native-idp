...
helm_chart_name = "Kasten"
helm_chart_namespace = "kasten-io"
helm_chart_instance_name = "k10"
...
class HelmService(Service):
    ...
    def InstallHelmChart(name="Install "+helm_chart_name):
        ...
        CalmTask.Exec.ssh(
            name="Configure K10 MultiCluster",
            filename="scripts/deploy_helm_chart/configure_k10_multicluster.sh",
            target=ref(HelmService),
            cred=ref(NutanixCred)
        )
...
class Default(Profile):
    ...
    primary_kasten_k8s_cluster = CalmVariable.WithOptions.FromTask(
        CalmTask.Exec.escript(
            name="",
            filename="../../../_common/karbon/scripts/get_karbon_cluster_list.py",
        ),
        label="Primary Kaster Cluster Name",
        is_mandatory=True,
        is_hidden=False,
        description="Primary Kasten Kubernetes Clusters for Multi-Cluster Management.",
    )
