...
OS_USERNAME = os.getenv("CALMDSL_OS_USERNAME")
OS_KEY = read_local_file("nutanix_key")

CRED_OS = basic_cred(OS_USERNAME, OS_KEY, name="CRED_OS", type="KEY", default=True,)

PC_USERNAME = os.getenv("CALMDSL_PC_USERNAME")

PC_PASSWORD = os.getenv("CALMDSL_PC_PASSWORD")
CRED_PC = basic_cred(
    PC_USERNAME, PC_PASSWORD, name="CRED_PC", type="PASSWORD", default=False,
)

PE_USERNAME = os.getenv("CALMDSL_PE_USERNAME")
PE_PASSWORD = os.getenv("CALMDSL_PE_PASSWORD")
CRED_PE = basic_cred(
    PE_USERNAME, PE_PASSWORD, name="CRED_PE", type="PASSWORD", default=False,
)

GCLOUD_ACCOUNT = read_local_file("gcloud_account")
GCLOUD_KEY = read_local_file("gcloud_key")
...
class ControlPlaneVMs_Substrate(Substrate):
    ...
    provider_spec.spec["name"] = "@@{ANTHOS_CLUSTER_NAME}@@-control-@@{calm_array_index}@@"
