...
docker-clean: #### Remove Calm DSL Util Image locally with necessary tools to develop and manage Cloud-Native Apps (e.g., kubectl, argocd, git, helm, helmfile, etc.)
	docker image ls --filter "reference=${IMAGE_REGISTRY_ORG}/cloud-native-calm-utils" --format "{{.Repository}}:{{.Tag}}" | xargs -I {} docker rmi -f {}
...
docker-build: docker-clean #### Build Calm DSL Util Image locally with necessary tools to develop and manage Cloud-Native Apps (e.g., kubectl, argocd, git, helm, helmfile, etc.)
	cd .devcontainer && docker build -t ${IMAGE_REGISTRY_ORG}/cloud-native-calm-utils:latest .
	docker tag ${IMAGE_REGISTRY_ORG}/cloud-native-calm-utils ${IMAGE_REGISTRY_ORG}/cloud-native-calm-utils:$(TARGET_RELEASE)

.PHONY: docker-run
docker-run: ### Launch into Calm DSL development container. If image isn't available, build will auto-run
	[ -n "$$(docker image ls ${IMAGE_REGISTRY_ORG}/cloud-native-calm-utils:$(TARGET_RELEASE) -q)" ] || make docker-build
	docker run --rm -it \
		...
		${IMAGE_REGISTRY_ORG}/cloud-native-calm-utils /bin/sh -c "make help && ${DEFAULT_SHELL}"
...
docker-release: docker-login ## Tag and Push latest release to desired image repo.
	@docker tag ${IMAGE_REGISTRY_ORG}/cloud-native-calm-utils ${IMAGE_REGISTRY_ORG}/cloud-native-calm-utils:$(GIT_LATEST_TAG)
	@docker push ${IMAGE_REGISTRY_ORG}/cloud-native-calm-utils:$(GIT_LATEST_TAG)
